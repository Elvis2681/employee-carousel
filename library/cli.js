...
class CLI {
  ...
  start() {
    this.db.init().then(() => this.mainMenu());
  }
  ...
  updateEmployeeRole() {
    ...
    return this.db
      ...
      .then((result) => {
        info.roles = result;
        const nameList = info.employees.map((item) => {
          return item.full_name;
        });
        const roleList = info.roles.map((item) => {
          return item.title;
        });
        return inquirer.prompt([
          {
  ...
  addNewEmployee() {
    ...
    return this.db
      ...
      .then((result) => {
        ...
        return this.db.idAndConcat();
      })
      .then((result) => {
        info.nameList = result;
        const roleList = info.roleList.map((item) => {
          return item.title;
        });
        const nameConcat = result.map((item) => {
          return item.full_name;
        });
  ...
  addNewRole() {
    ...
    return this.db
      ...
      .then((result) => {
        info.dept = result;
        const deptNameList = result.map((item) => {
